package org.java_tutorials.part_1.measurement.polygon;
...
abstract public class Polygon implements PolygonMeasurement {
    ...
    public Polygon(String... vertex) {
        Arrays.stream(vertex).forEachOrdered(s -> {
            this.xCoordinate.add(Double.parseDouble(s.substring(1, s.indexOf(','))));
            this.yCoordinate.add(Double.parseDouble(s.substring(s.indexOf(',') + 1, s.indexOf(')'))));
        });
    }
    ...
    public TreeMap<String, Double> getDiagonals() {
        ...
        for (int i = 0; i < xCoordinate.size(); i++) {
            ...
            if (i != 0 && i != 1)
                k--;
            for (int j = 1; j <= k; j++) {
                hashMap.put("D" + (i + 1), Math.sqrt(Math.pow((xCoordinate.get(i) -
                        xCoordinate.get(point)), 2)
    ...
    public TreeMap<String, Double> getAngles() {
        ...
        for (int i = 0; i < xCoordinate.size(); i++) {
            ...
            if (concaveVertex.contains(i)
                    || (concaveVertex.contains(xCoordinate.size()) && i == xCoordinate.size() - 1))
    ...
    public double getDistanceBetween(int v1, int v2) {
        return Math.sqrt(Math.pow((xCoordinate.get(v2) - xCoordinate.get(v1)), 2)
                + Math.pow(((yCoordinate.get(v2) - yCoordinate.get(v1))), 2));
    }
